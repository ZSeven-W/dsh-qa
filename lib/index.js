...
var PREDICATE_FIELDS = ["role", "name", "tag", "roleHint"];
...
function buildStepResult(base, assertion, receipt, outcome, assertionPassed, observed, completeness = null, reason, attempts, elapsedMs, scope = {}, targetResolution = null, targetChangedRetry = false) {
  return {
    index: base.index,
    intent: base.intent,
    ...
    ...targetChangedRetry ? { targetChangedRetry: true } : {}
  };
}
...
function durableAction(recorded, before) {
  ...
  const nameOnly = name2 !== void 0 && countMatches(before, { name: name2 }) === 1;
    ...
    if (step.targetChangedRetry === true) {
      lines.push(
        "  - target changed retry: " + mdInline(
          "the driver refused the first dispatch with TARGET_CHANGED (the page replaced the bound element mid-flight); the runner re-observed, re-resolved the same semantic target, and dispatched once more"
        )
      );
    }
