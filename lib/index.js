...
function settleStartOverride(args) {
  ...
  if (budgetMs === void 0 && quietMs === void 0) return void 0;
  return { ...budgetMs === void 0 ? {} : { budgetMs }, ...quietMs === void 0 ? {} : { quietMs } };
}
...
async function decideAssertionWithRetry(assertion, observation, reobserve, budgetMs) {
  const startedAt = Date.now();
  let attempts = 1;
...
var SETTLE_OVERRIDE_FIELDS = ["budgetMs", "quietMs", "postChangeQuietMs", "intervalMs"];
...
function scenarioSettleOverride(policy) {
  ...
  if (policy.budgetMs === defaults.budgetMs && policy.quietMs === defaults.quietMs && policy.postChangeQuietMs === defaults.postChangeQuietMs && policy.intervalMs === defaults.intervalMs) {
    return void 0;
  }
...
function renderReportMarkdown(report, roots) {
  ...
  if (report.settle !== void 0) {
    ...
    lines.push(
      "- **Settle policy**: budget " + String(settle.budgetMs) + "ms, quiet " + String(settle.quietMs) + "ms, post-change quiet " + String(settle.postChangeQuietMs) + "ms, interval " + String(settle.intervalMs) + "ms"
    );
  }
  lines.push("");
  lines.push("## Steps");
