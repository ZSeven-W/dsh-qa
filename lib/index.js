...
var ComputerAdapter = class {
   ...
   */
  async visualObserve(ownerId, observationId) {
...
var ROOT_FIELDS = ["meta", "target", "steps", "assertions"];
var META_FIELDS = ["name", "description", "driver", "createdAt"];
...
function validateMeta(value, position) {
  ...
  return { name: name2, description, driver, createdAt };
}
...
function skipAuthWhitespace(text, index) {
  let i = index;
...
function scanCredentialKeySuffixSeparator(text, punctuationPos) {
  if (punctuationPos >= text.length || punctuationPos + 1 >= text.length || !isCredentialKeySuffixPunctuationCode(text.charCodeAt(punctuationPos)) || !isAuthWhitespaceCode(text.charCodeAt(punctuationPos + 1))) {
    return -1;
  }
...
function scanAuthorizationWholeValueEnd(text, start, stopCode) {
  ...
  while (i < length) {
    ...
    if (code === 10) {
      ...
      if (openQuote !== "" || next === 32 || next === 9 || isAsciiWordCharCode(next)) {
        i += 1;
        continue;
      }
      break;
...
function consumeCredentialValue(text, valueStart, keyQuote, keyText, separator, stopCode) {
  ...
  while (true) {
    ...
    if (credentialSensitive && currentValueStart < text.length && (text.charCodeAt(currentValueStart) === 61 || text.charCodeAt(currentValueStart) === 58)) {
...
function isArtifactPathPosition(segments) {
  return segments.length === 3 && segments[0] === "artifacts" && typeof segments[1] === "number" && segments[2] === "path";
}
...
function renderReportMarkdown(report, roots) {
  ...
  for (const assertion of report.assertions) {
    lines.push(
      "- " + md(assertion.kind) + " -> " + (assertion.passed ? "PASS" : "FAIL") + " (observed: " + TICK + md(inline(assertion.observed, roots)) + TICK + ")"
    );
  }
...
import { mkdir, appendFile as appendFile2, writeFile } from "node:fs/promises";
...
var QA_SKILL_DESCRIPTION = "Drive acceptance runs with the qa_* tools: start a browser or computer session, observe, act, assert, collect evidence, and replay deterministic scenarios. Read this before the first qa_* call of a QA task.";
var QA_SKILL_WHEN_TO_USE = "Any task that operates an app through the qa_* tools \u2014 starting a session, observing the UI, performing an action, asserting on state, collecting evidence, or replaying a recorded scenario \u2014 over a Browser (BU) or Computer (CU) driver.";
var QA_SKILL_CONTENT = `# QA with dsh-qa
