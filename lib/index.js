...
var ASSERTION_SCOPE_PATH_ITEM_FIELDS = ["role", "name"];
...
function ancestorPathIn(view, node) {
...
function sameScopePath(recorded, candidate) {
  if (recorded === void 0) return true;
  if (candidate === null) return false;
  return recorded.length === candidate.length && recorded.every((item, index) => item.role === candidate[index].role && item.name === candidate[index].name);
}
async function observeScopeView(scope, wholePage, session, reobserve, options = {}) {
  ...
  const matchesIn = (view2) => view2.nodes.filter(
    (node) => matchesNode(node, predicate) && sameScopePath(path2, ancestorPathIn(view2, node))
  );
  let view = wholePage;
  ...
  if (matches.length === 0) {
    ...
    throw new Error(
      "no observable node matches the assertion scope (" + describeScope(scope) + (path2 === void 0 ? "" : ", ancestor path " + JSON.stringify(path2)) + ")"
    );
  }
  if (matches.length > 1) {
  ...
  if (view.truncated && options.provisionalScope !== true && path2 === void 0) {
    const applied = escalated ? view.maxNodes === void 0 ? "the escalated budget (the driver did not report the budget it applied)" : "the applied " + String(view.maxNodes) + "-node escalated budget" : "the driver-default budget";
    throw new Error(
  ...
  const settled = await session.observeSettled({
    ...
    ...options.scopedObserve === void 0 ? {} : options.scopedObserve
  });
  if (!settled.stable) {
...
function provisionalScopeCompleteness(assertion, scoped) {
  ...
  return {
    ...
    detail: "the scoped container was resolved PROVISIONALLY: exactly one observable node matched the assertion scope" + (assertion.scope?.path === void 0 ? "" : " (the recorded ancestor path matched)") + " in a still-truncated whole-page view, so the container's uniqueness is unproven and the " + assertion.kind + " assertion was NOT decided as proven (" + QA_INCONCLUSIVE_SCOPE + ")."
  };
}
...
async function resolveStepAction(action, assert, wholePage, session, reobserve, options = {}) {
...
function decideScopedScrollProof(assertion, target, scoped) {
  ...
  if (matches.length === 0) {
    if (view.truncated) {
      return {
        kind: "failure",
        error: new Error(
          "no observable node matches the scroll target inside the scoped container, and the scoped view was still truncated at the applied budget (" + QA_INCONCLUSIVE_TRUNCATED + "): the target may exist outside the returned subtree window rather than be missing from the page"
        )
      };
    }
    return {
      kind: "failure",
      error: new Error(
        "the scoped scroll-proof assertion found no observable node matching the target (" + describeTarget(target) + ") inside the complete scoped container"
      )
    };
  }
  if (matches.length > 1) {
  ...
  if (scoped.resolution === "proven" && subtreeComplete && coverageVerified && targetNode.inViewport === true) {
    return {
      ...
      completeness: {
        ...completenessBase,
        detail: "the container was resolved in a complete whole-page view (proven), the scoped subtree is complete with verified coverage, and the identity anchor confirmed the asserted target is the anchored node in the viewport."
      }
    };
  }
  const why = scoped.resolution === "provisional" ? "the scoped container was resolved PROVISIONALLY (one match in a still-truncated whole-page view)" : subtreeComplete ? "the scoped subtree was complete but its coverage was not verified (coverage.verified !== true), so target uniqueness inside the scope is unproven" : "the scoped subtree was still truncated at its budget, so target uniqueness inside the scope is unproven";
  return {
    kind: "inconclusive",
    observed: evaluation.observed,
    completeness: {
...
function buildStepResult(base, assertion, receipt, outcome, assertionPassed, observed, completeness = null, reason, attempts, elapsedMs, scope = {}) {
  return {
    index: base.index,
    intent: base.intent,
    ...
    if (failure === null) {
      ...
      for (let i = 0; failure === null && i < scenario.assertions.length; i += 1) {
        ...
        if (lastScrollProofStep !== null && lastScrollProofDecision !== null) {
          ...
          const isCopy = assertion.kind === "node-in-viewport" && assertion.scope !== void 0 && sameJsonShape(assertion.scope, scrollProofStep.assert.scope) && sameJsonShape(
            assertion.expected,
            scrollProofStep.action.target
          );
          if (isCopy) {
        ...
        const provisionalScope = lastScrollProofStep !== null && assertion.kind === "node-in-viewport" && assertion.scope !== void 0 && sameJsonShape(assertion.scope, lastScrollProofStep.assert.scope) && sameJsonShape(
          assertion.expected,
          lastScrollProofStep.action.target
        );
        let decision;
        try {
          decision = await decideAssertionScoped(
...
function recordedContainerPath(observations, baseline, role, name2, tag) {
  ...
  for (const observation of sources) {
    ...
    for (const node of observation.nodes) {
      ...
      const path2 = ancestorPathOf(observation, node);
...
function proofScope(proof, baseline, anchored, path2) {
  if (proof === null || proof.scope === void 0) return null;
  const echo = proof.scope;
  ...
  const provisionalWeakness = (why) => "the scope is PROVISIONAL: " + why + " The step is an identity-anchored scroll proof, so the scope is exported explicitly provisional; replay reports INCONCLUSIVE_SCOPE (scopeResolution provisional) unless it proves the container unique at replay time.";
  const unproven = (why) => {
    if (!anchored) {
      return {
        durable: false,
...
function withProofScope(assertion, proof, baseline, anchored, path2) {
  if (assertion.kind === "page-url") return { assertion, notDurable: null, weakness: null };
  const verdict = proofScope(proof, baseline, anchored, path2);
...
function buildScenario(trajectory, options) {
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    if (isDirectionScroll) {
      ...
      const scopedStep2 = withProofScope(resolved.assert, candidate.after, candidate.before, anchored2, scopePath2);
...
function renderReportMarkdown(report, roots) {
  ...
  for (const step of report.steps) {
    ...
    lines.push(
      "  - assertion: " + mdInline(step.assertion.kind) + " -> " + (step.assertionPassed ? "PASS" : step.reason === QA_INCONCLUSIVE_SCOPE ? "INCONCLUSIVE" : "FAIL")
    );
  ...
  for (const assertion of report.assertions) {
    lines.push(
      "- " + mdInline(assertion.kind) + " -> " + (assertion.passed ? "PASS" : assertion.reason === QA_INCONCLUSIVE_SCOPE ? "INCONCLUSIVE" : "FAIL") + " (observed: " + mdCode(inline(assertion.observed, roots)) + ")"
    );
