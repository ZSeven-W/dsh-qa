...
async function decideAssertion(assertion, observation, reobserve) {
  ...
  if (!observation.truncated && observation.scope === void 0) {
    return {
      passed: first.passed,
...
var SCROLL_TARGET_CONTAINER_ROLES = /* @__PURE__ */ new Set([
  "region",
  "main",
  "navigation",
  "list",
...
function scrollTargetContainer(baseline, targetIndex) {
  const nodes = baseline.nodes;
  for (let index = targetIndex - 1; index >= 0; index -= 1) {
    const candidate = nodes[index];
    if (candidate !== void 0 && SCROLL_TARGET_CONTAINER_ROLES.has(candidate.role)) {
...
function scopedScrollProofConsistent(settled, scoped) {
  ...
  for (const scopedNode of scoped.nodes) {
    for (const settledNode of settled.nodes) {
      if (settledNode.role === scopedNode.role && settledNode.name === scopedNode.name && settledNode.tag === scopedNode.tag && !scrollProofNodeEquivalent(settledNode, scopedNode)) {
        return false;
      }
    }
  }
...
var QaSession = class {
  ...
  async #escalateScrollProof(action, baselineObservation, settledObservation, actionId) {
    ...
    const container = baselineObservation === null ? null : scrollTargetContainer(baselineObservation, targetIndex);
    if (container !== null) {
      const containerNode = settledObservation.nodes.find(
        (candidate) => candidate.role === container.role && candidate.name === container.name && candidate.tag === container.tag
      );
      if (containerNode !== void 0) {
        try {
          ...
          if (escalated.stable && scoped.scope !== void 0 && scopedScrollProofConsistent(settledObservation, scoped) && targetInViewport) {
            try {
...
var ASSERTION_SCOPE_FIELDS = ["role", "name"];
...
async function observeScopeView(scope, wholePage, session, reobserve) {
  const matchesIn = (view2) => view2.nodes.filter((node) => matchesNode(node, { role: scope.role, name: scope.name }));
  let view = wholePage;
  ...
  if (matches.length > 1) {
    throw new QaCodeError(
      QA_TARGET_NOT_UNIQUE,
      QA_TARGET_NOT_UNIQUE + ": " + String(matches.length) + ' nodes match the assertion scope (role "' + scope.role + '", name "' + scope.name + '"); the scoped container is not uniquely identifiable, so the assertion was not decided'
    );
  }
...
function proofScope(observation) {
  if (observation === null || observation.scope === void 0) return null;
  const role = clean(observation.scope.role);
  const name2 = clean(observation.scope.name);
  if (role === "" || name2 === "") return null;
  return { role, name: name2 };
}
function withProofScope(assertion, proof) {
...
function scopedPrecedingViewWeakness(before) {
  ...
  return "the action's preceding observation was scoped to the " + scope.role + ' named "' + scope.name + `", so the action target's whole-page uniqueness was not verified at export (Replay resolves the target in the whole-page view) \u2014 verify manually.`;
}
