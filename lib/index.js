...
function completenessDetail(kind, deciding, truncated, nodeBudget, priorBudget, truncationReasons, escalated, escalationFailed) {
  const context = {
    kind,
    deciding,
    truncated,
  ...
  if (deciding.inconclusive) {
    return escalation + "the view was STILL truncated at its " + budget + ' budget, so "' + kind + '" cannot be proven from it: a matching node may exist outside the returned window. This is not "not present" \u2014 ' + truncationAdvice(context) + " then re-run.";
  }
  if (truncated) {
