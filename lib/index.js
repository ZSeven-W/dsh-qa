...
var RETRY_FIELDS = /* @__PURE__ */ new Set(["attempts", "elapsedMs", "targetChangedRetries", "message"]);
...
function identityExhaustionMessage(retries, widened) {
  return "the target kept changing identity between resolution and dispatch for the whole settle budget (" + String(retries) + " retries): the page did not hold still, so the step is unproven" + (widened === null ? "" : " \u2014 the settle budget was widened once from " + String(widened.fromMs) + " to " + String(widened.toMs) + "ms and still exhausted");
}
...
async function observeScopeView(scope, wholePage, session, reobserve, options = {}) {
  ...
  if (path2 !== void 0 && path2.length > 0) {
    for (let index = 0; index < path2.length; index += 1) {
      ...
      const settled3 = await session.observeSettled({ withinRef: resolved.node.ref });
      if (!settled3.stable) {
        throw new Error(unsettledMessage("scoped", settled3.budgetMs));
      }
      view = settled3.observation;
    ...
    const container2 = await resolveLevel(
    ...
    const settled2 = await session.observeSettled({
      withinRef: container2.node.ref,
      ...options.scopedObserve === void 0 ? {} : options.scopedObserve
    });
    if (!settled2.stable) {
      throw new Error(unsettledMessage("scoped", settled2.budgetMs));
    }
    return {
      kind: "resolved",
      observation: settled2.observation,
    ...
    for (let stepIndex = 0; stepIndex < scenario.steps.length; stepIndex += 1) {
      ...
      try {
        ...
        while (result.outcome === "failed" && result.receipt.code === "TARGET_CHANGED") {
          ...
          if (!refresh.stable) {
            identityExhausted = "the post-TARGET_CHANGED refresh observation never settled within the " + String(refresh.budgetMs) + "ms settle budget (after " + String(targetChangedRetries) + " retries): the page did not hold still, so the step is unproven";
            break;
          }
...
function pathItemFor(ancestor) {
  ...
  const name2 = clean(ancestor.name);
...
function pathItemMatchesNode(item, node) {
  ...
  if (item.name !== void 0 && clean(node.name) !== item.name) return false;
  if (item.tag !== void 0 && clean(node.tag) !== item.tag) return false;
  return true;
}
function scopeEchoesPathItem(view, item) {
  ...
  if (item.name !== void 0 && clean(scope.name) !== item.name) return false;
  if (item.tag !== void 0 && clean(scope.tag) !== item.tag) return false;
  return true;
}
...
function renderReportMarkdown(report, roots) {
    ...
    if (step.targetChangedRetries !== void 0) {
      lines.push(
        "  - target changed retries: " + String(step.targetChangedRetries) + " \u2014 " + mdInline(
          "the driver refused the dispatch with TARGET_CHANGED (the page replaced the bound element between resolution and dispatch) " + String(step.targetChangedRetries) + " time(s); the runner re-observed, re-resolved the same semantic target, and re-dispatched within the settle budget"
        )
      );
    }
  ...
  for (const assertion of report.assertions) {
    lines.push(
      "- " + mdInline(assertion.kind) + " -> " + (assertion.passed ? "PASS" : assertion.reason === QA_INCONCLUSIVE_SCOPE || assertion.scopeNotLocated === true ? "INCONCLUSIVE" : "FAIL") + " (observed: " + mdCode(inline(assertion.observed, roots)) + ")"
    );
