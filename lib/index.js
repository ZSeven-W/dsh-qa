...
function matchesEcho(node, echo) {
  if (echo.role !== void 0 && node.role !== echo.role) return false;
  if (echo.name !== void 0 && node.name !== echo.name) return false;
  if (echo.tag !== void 0 && node.tag !== echo.tag) return false;
  return true;
}
function projectSemanticView(observation, echo) {
  return JSON.stringify({
    ...
    nodes: observation.nodes.map((node) => [
      ...
      node.secure ?? null,
      echo !== void 0 && matchesEcho(node, echo) ? null : node.value ?? null
    ])
  });
}
function sleep(ms) {
...
function actionEcho(action, before) {
  ...
  if (predicate.role === void 0 && predicate.name === void 0 && predicate.tag === void 0) {
    return null;
  }
  return predicate;
}
...
function buildStepResult(base, assertion, receipt, outcome, assertionPassed, observed, completeness = null) {
  return {
    index: base.index,
    intent: base.intent,
...
var QA_TOOL_DESCRIPTIONS = {
  ...
  qa_replay_run: "Run a deterministic Replay scenario file end to end and return a pass/fail/blocked report. Browser scenarios only in v0.1.",
