...
import { mkdir, appendFile as appendFile2, writeFile as writeFile2 } from "node:fs/promises";
import { join as join2 } from "node:path";
async function writeReports(report, options) {
  await mkdir(options.directory, { recursive: true });
  const jsonPath = join2(options.directory, "report.json");
  const markdownPath = join2(options.directory, "report.md");
  const jsonlPath = options.jsonlPath ?? join2(options.directory, "report.jsonl");
  await writeFile2(jsonPath, renderReportJson(report, options.roots), "utf8");
  await writeFile2(markdownPath, renderReportMarkdown(report, options.roots), "utf8");
