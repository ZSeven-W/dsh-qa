...
async function observeUntilStable(observe, policy = resolveSettlePolicy(), options = {}) {
...
function completenessDetail(kind, deciding, truncated, nodeBudget, escalated, escalationFailed) {
  const budget = budgetLabel(nodeBudget);
  const escalation = escalationFailed ? "the bounded escalation to the " + String(QA_ESCALATED_NODE_BUDGET) + "-node budget could not be observed, so the outcome was decided against the truncated view; " : escalated ? "one bounded re-observation at the " + String(QA_ESCALATED_NODE_BUDGET) + "-node budget was taken; " : "";
  if (!readsNodes(kind)) {
  ...
  if (deciding.inconclusive) {
    return escalation + "the view was STILL truncated at its " + budget + ' budget, so "' + kind + '" cannot be proven from it: a matching node may exist outside the returned window. This is not "not present" \u2014 raise the observation node budget (qa_observe max_nodes) or narrow the page, then re-run.';
  }
  if (truncated) {
    return escalation + "the deciding view was truncated at its " + budget + " budget, but a matching node was RETURNED by it, and a returned node is sound evidence of presence even in an incomplete view.";
  }
  return escalation + "the first view was truncated at its node budget; the deciding view at the " + budget + " budget was COMPLETE, so the outcome is proven against the whole view.";
}
...
function failureCodeFor(error) {
  return error instanceof QaCodeError ? error.code : void 0;
}
...
async function resolveActionWithBudget(action, observation, reobserve) {
  ...
  if (missing(view)) {
    if (view.truncated) {
      throw new Error(
        "no observable node matches the action target, and the view was still truncated at the " + (escalated ? String(QA_ESCALATED_NODE_BUDGET) + "-node escalated" : "driver-default") + " budget (" + QA_INCONCLUSIVE_TRUNCATED + "): the target may exist outside the returned window rather than be missing from the page"
      );
    }
    throw new Error("no observable node matches the action target");
  }
...
function buildStepResult(base, assertion, receipt, outcome, assertionPassed, observed, completeness = null, reason) {
  return {
    index: base.index,
    intent: base.intent,
...
async function runScenario(scenario, adapter, options = {}) {
  ...
  const session = new QaSession(adapter, ownerId, {
    ...options.settle === void 0 ? {} : { settle: options.settle }
  });
  const stepResults = [];
...
function semanticDelta(before, after, target) {
  ...
  const candidates = after.nodes.map((node, order) => ({ node, order, predicate: predicateFor(node) })).filter((item) => item.predicate !== null && countMatches(after, item.predicate) === 1 && countMatches(before, item.predicate) === 0).map((item) => ({
    ...item,
...
function synthesizeValueAssertion(after, target, action) {
  ...
  if (node === void 0) {
    const renamed = after.nodes.find((candidate) => target.role !== void 0 && candidate.role === target.role && typeof candidate.value === "string" && candidate.value === expected && candidate.valueWithheld !== true && candidate.secure !== true && candidate.valueTruncated !== true);
...
function buildScenario(trajectory, options) {
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    if (isDirectionScroll) {
      ...
      steps.push({
        ...
        intent: normalizeIntent(intentWithWeaknesses(
          resolved.intent,
          candidate.after !== null && proofWasTruncated(candidate.before, candidate.after) ? [TRUNCATED_PROOF_WEAKNESS] : []
        )),
    ...
    const synthesized = stepAction.kind === "scroll" && candidate.target !== null ? { assertion: synthesizeScrollAssertion(candidate.target), weakness: null } : synthesizeAssertion(candidate.before, after, candidate.target, stepAction);
    if (synthesized === null || !evaluateAssertion(synthesized.assertion, after).passed) {
      excluded.push(exclusion(
        recorded,
        "ASSERTION_NOT_PROVABLE",
        receipt?.status === "unknown" ? "Unknown receipt had no semantic state change in the settled observation." : "The settled observation had no semantic state change or URL change proving the action outcome."
      ));
      continue;
    }
...
function completenessLine(completeness) {
  return (completeness.reason === void 0 ? "" : completeness.reason + " \u2014 ") + "view truncated: " + String(completeness.truncated) + ", node budget: " + (completeness.nodeBudget === null ? "driver default" : String(completeness.nodeBudget)) + ", budget escalated: " + String(completeness.escalated) + ", outcome depends on a complete view: " + String(completeness.outcomeDependsOnCompleteView) + ". " + completeness.detail;
}
var NEWLINE_MARK = "\u23CE";
...
var QA_TOOL_DESCRIPTIONS = {
  ...
  qa_record_export: "Export this owner's redacted Explore trajectory as a fail-closed Replay scenario JSON file. Only actions with durable role+accessible-name targets and outcomes proven by immediate fresh observations become steps; exclusions are returned explicitly.",
  qa_replay_run: "Run a deterministic Replay scenario file end to end and return a pass/fail/blocked report. Browser scenarios only in v0.1. A failure carries a machine failure.code for recognized non-results (INCONCLUSIVE_UNSTABLE for a view that never settled, TARGET_NOT_UNIQUE for an ambiguous action target), so prose never has to be parsed to tell them from an ordinary assertion failure.",
