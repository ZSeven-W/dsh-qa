...
function assertDriverContract(driver, supported, packageName, hint) {
  ...
  throw new Error(
    "Refusing to use " + packageName + ": it reports driver contract version " + describe + ", and this build of @zseven-w/dsh-qa supports exactly version " + String(supported) + ". A different contract can change what an observation means, so the evidence behind every verdict would be uninterpretable rather than merely degraded. " + hint
  );
}
async function loadBrowserManager(specifier = BROWSER_DRIVER_SPECIFIER, options, deps) {
  try {
    const importModule = deps?.importModule ?? ((id) => import(id));
...
var BROWSER_DRIVER_SPECIFIER, SUPPORTED_BROWSER_CONTRACT_VERSION;
var init_loadBrowser = __esm({
  "src/adapters/loadBrowser.ts"() {
    "use strict";
    BROWSER_DRIVER_SPECIFIER = "@zseven-w/dsh-browser";
    SUPPORTED_BROWSER_CONTRACT_VERSION = 9;
  }
});
...
async function loadComputerDriver(specifier = COMPUTER_DRIVER_SPECIFIER, deps) {
  try {
    const importModule = deps?.importModule ?? ((id) => import(id));
...
var COMPUTER_DRIVER_SPECIFIER, SUPPORTED_COMPUTER_CONTRACT_VERSION;
var init_loadComputer = __esm({
  "src/adapters/loadComputer.ts"() {
    ...
    SUPPORTED_COMPUTER_CONTRACT_VERSION = 5;
  }
});
...
var QA_SCENARIO_SCHEMA_VERSION = 1;
...
function adaptiveEnvValue(suffix) {
  const raw = pluginEnv(suffix);
  if (raw === void 0 || raw.trim() === "") return void 0;
...
function resolveAdaptiveBudgetMs(explicit, budgetMs) {
  const raw = explicit !== void 0 ? explicit : adaptiveEnvValue("QA_SETTLE_ADAPTIVE_BUDGET_MS") ?? QA_SETTLE_ADAPTIVE_BUDGET_MS;
  if (raw === 0) return 0;
...
function resolveSettlePolicy(options) {
  ...
  const postChangeRaw = options?.postChangeQuietMs ?? fromEnv("QA_SETTLE_POST_CHANGE_QUIET_MS") ?? 2 * quietMs;
...
function matchesStructureCloser(open2, close) {
  return open2 === 91 && close === 93 || open2 === 123 && close === 125;
}
function scanStructuredCredentialValue(text, start) {
  ...
  let end = -1;
  for (; ; ) {
    const open2 = text.charCodeAt(i);
    if (!isOpenStructureCharCode(open2)) {
      break;
    }
    const stack = [open2];
    let j = i + 1;
...
var ROOT_FIELDS = ["schemaVersion", "meta", "target", "steps", "assertions", "advisory"];
...
function validateSchemaVersion(value, position) {
  if (value === void 0) return void 0;
  if (typeof value !== "number" || !Number.isInteger(value) || value < 1) {
    fail2(position, "schemaVersion must be a positive integer");
  }
  if (value > QA_SCENARIO_SCHEMA_VERSION) {
    fail2(
      position,
      "schemaVersion " + String(value) + " is newer than this build understands (" + String(QA_SCENARIO_SCHEMA_VERSION) + "). Replaying it under the current rules could report a confident verdict for a scenario whose meaning has changed; upgrade @zseven-w/dsh-qa instead."
    );
  }
  return value;
}
...
function stripSingleWholeFence(text) {
  const trimmed = text.trim();
  const open2 = /^(`{3,}|~{3,})/.exec(trimmed);
  const openFence = open2?.[1];
...
function isUnprovenDecision(decision) {
  const unproven = /* @__PURE__ */ new Set([
  ...
  if (decision.reason !== void 0 && unproven.has(decision.reason)) return true;
  const completenessReason = decision.completeness?.reason;
  return completenessReason !== void 0 && unproven.has(completenessReason);
}
function decideScopedScrollProof(assertion, target, scoped) {
  ...
  if (!evaluation.passed) {
    const detail = fullyProven ? "the scoped scroll was anchor-verified, but the step's own assertion does NOT hold in that view \u2014 the container was resolved PROVEN (" + levelSummary(scoped.levels) + "), the subtree is complete with verified coverage, so the assertion is DISPROVEN, not merely unproven." : "the step's own assertion does not hold in the scoped view, and the view is not good enough to disprove it either (" + QA_INCONCLUSIVE_SCOPE + ").";
    return fullyProven ? { kind: "disproven", observed: evaluation.observed, completeness: { ...completenessBase, detail }, targetResolution } : { kind: "inconclusive", observed: evaluation.observed, completeness: { ...completenessBase, detail }, targetResolution };
  }
...
function describeAssertionForExclusion(assertion) {
  const expected = assertion.expected;
  const role = typeof expected?.role === "string" ? expected.role : null;
  const name2 = typeof expected?.name === "string" ? expected.name : null;
  const identity = [
    role === null ? null : "role " + JSON.stringify(role),
    name2 === null ? null : "name " + JSON.stringify(name2)
  ].filter((part) => part !== null).join(", ");
  return assertion.kind + (identity === "" ? "" : " (" + identity + ")");
}
function synthesizeValueAssertion(before, after, target, action) {
  ...
  if (node === void 0) {
    const heldBefore = (candidate) => {
      if (before === null) return false;
      return before.nodes.some((prior) => prior.role === candidate.role && prior.name === candidate.name && typeof prior.value === "string" && prior.value === expected);
    };
    const candidates = after.nodes.filter((candidate) => typeof candidate.value === "string" && candidate.value === expected && candidate.valueWithheld !== true && candidate.secure !== true && candidate.valueTruncated !== true && !heldBefore(candidate) && (target.name !== void 0 && candidate.name === target.name || target.role !== void 0 && candidate.role === target.role));
  ...
  for (const recorded of trajectory.assertions) {
    if (!recorded.passed) {
      excludedAssertions.push({
        ...
        detail: "the recorded qa_assert did NOT hold when it was decided, so it is not exported as a scenario assertion: " + describeAssertionForExclusion(recorded.assertion)
      });
      continue;
    }
...
var REPORT_MODE = 384;
async function openReportSink(path2, append) {
  ...
  let handle;
  try {
    handle = await open(path2, flags, REPORT_MODE);
  } catch (error) {
    const code = error.code;
    if (code === "ELOOP" || code === "ENXIO" || code === "EOPNOTSUPP") {
      throw new Error(
        "refusing to write the report to " + path2 + ": it is not a regular file (a symlink, FIFO or device would deliver the report somewhere the caller did not name). Remove it or choose another output directory. Underlying code: " + String(code)
      );
    }
    throw error;
  }
  ...
  if (!info.isFile()) {
    await handle.close();
    throw new Error(
      "refusing to write the report to " + path2 + ": it is not a regular file. Remove it or choose another output directory."
    );
  }
  return handle;
}
async function writeSink(path2, text, append) {
  const handle = await openReportSink(path2, append);
  try {
