...
function projectSemanticView(observation) {
...
async function observeUntilStable(observe, policy = resolveSettlePolicy(), options = {}) {
  ...
  let changed = options.baselineView !== void 0 && options.baselineView !== projection;
  let passes = 1;
  for (; ; ) {
    const now = Date.now();
...
var ASSERTION_KINDS = ["node-present", "node-absent", "page-url", "node-in-viewport"];
...
function synthesizeAssertion(before, after, target) {
...
function buildScenario(trajectory, options) {
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    const synthesized = stepAction.kind === "scroll" && candidate.target !== null ? { assertion: synthesizeScrollAssertion(candidate.target), weakness: null } : synthesizeAssertion(candidate.before, after, candidate.target);
