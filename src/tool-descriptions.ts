...
export const QA_TOOL_DESCRIPTIONS = {
  ...
  qa_replay_run:
    'Run a deterministic Replay scenario file end to end and return a pass/fail/blocked report. Browser scenarios only in v0.1. Positive existence assertions (node-present/node-value/node-in-viewport/page-url) are re-observed within the settle budget when first not found (a slow page), with attempts/elapsedMs recorded in the step result and report.md; node-absent is never retried into a pass. A failure carries a machine failure.code for recognized non-results (INCONCLUSIVE_UNSTABLE for a view that never settled, TARGET_NOT_UNIQUE for an ambiguous action target), so prose never has to be parsed to tell them from an ordinary assertion failure.',
